...
data.org <- aggregate(data.org$active, by=list(data.org$side_b_new,data.org$datum),FUN=sum)
	colnames(data.org) <- c("side_b_new","datum","prio.grids.count")
...
prio.grids.count <- merge(prio.grids.count,uniq.org,by=c("side_b_new"),all.x=TRUE)

save(prio.grids.count,file="~/Documents/git/rebelTrack/Rcode/area_prio_grid/prio.grids.count.qt.rda")


# pdf(file="area_grid.pdf",height=5,width=5*1.65)
# plot(prio.grids.count$datum[prio.grids.count$side_b_new==327],prio.grids.count$prio.grids.count[prio.grids.count$side_b_new==327],type="l",axes=F,ann=F)
# axis(1)
# axis(2,las=T)
# title(main="side_b_new=327",xlab="Months",ylab="PRIO grids affected")
# dev.off()
